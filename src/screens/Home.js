...
function Data(e)
{
 ...
 fooditems.map((i)=>{
    ...
    if(i.name===name)
      {
        alert(`${name} is added in the cart`)
 ...
 function login()
 {
  ...
  return (
    <div onClick={login}>
      ...
      <button onClick={toggleModal} className=" btn-modal btn btn-dark" disabled>Click to open cart<sup >{cart.length}</sup></button>
      ...
      {fooditems.map((i)=>{
        ...
        return (
          <div>
           ...
           <button className="btn btn-light" key={i.id} onClick={()=>handelCart(i.name)} disabled>ADD TO CART</button>
